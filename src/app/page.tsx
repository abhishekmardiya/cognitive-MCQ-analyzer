...
function sortEvaluations(list: McqEvaluation[]) {
  return [...list].sort((a, b) => {
    return a.index - b.index;
  });
}

function inputStatusBadgeClasses(
...
function inputStatusLabel(status: McqEvaluationResult["inputStatus"]): string {
  switch (status) {
    case "complete":
      return "Complete";
    case "incomplete":
              ...
              <div className="min-h-10 flex flex-1 items-center border-t border-zinc-200 pt-3 sm:border-t-0 sm:border-l sm:pl-4 sm:pt-0 dark:border-zinc-700">
                ...
                {!pendingPdfFile && !plainTextSourceName ? (
                  <p className="text-sm text-zinc-500 dark:text-zinc-400">
                    No file selected · .txt, .md, or .pdf (max {MAX_UPLOAD_MB}{" "}
                    MB)
                  </p>
                ) : null}
              </div>
